...
function renderVc1Overview(d, clickedTicker){
  ...
  const fs=d.feature_summary||{}, fcat=fs.by_category||[], fst=d.feature_stats||[];
  const fTbl=fst.length
    ? `<table style="width:100%;border-collapse:collapse;margin-top:6px;font-size:11px"><thead><tr style="color:var(--muted);text-align:left;border-bottom:1px solid #333"><th style="padding:3px 6px;text-align:right">#</th><th style="padding:3px 6px">Feature</th><th style="padding:3px 6px">Cat</th><th style="padding:3px 6px;text-align:right" title="avg |weight| across walk-forward windows">Avg|Wt|</th><th style="padding:3px 6px;text-align:right">Signed</th><th style="padding:3px 6px;text-align:right" title="t-test of weights vs 0">p-val</th><th style="padding:3px 6px;text-align:right">Wins</th><th style="padding:3px 6px;text-align:right" title="% windows same sign">Sign%</th><th style="padding:3px 6px;text-align:right">StdErr</th><th style="padding:3px 6px;text-align:right" title="std / |mean|">Instab</th></tr></thead><tbody>${fst.map(r=>`<tr style="border-bottom:1px solid #161616"><td style="padding:3px 6px;text-align:right;color:var(--muted)">${r.rank}</td><td style="padding:3px 6px;font-family:monospace;color:#cbd5e1">${r.feature}</td><td style="padding:3px 6px;color:var(--muted)">${r.category}</td><td style="padding:3px 6px;text-align:right;font-family:monospace">${r.avg_abs.toFixed(3)}</td><td style="padding:3px 6px;text-align:right;font-family:monospace;color:${r.signed>=0?'var(--green)':'var(--red)'}">${r.signed>=0?'+':''}${r.signed.toFixed(3)}</td><td style="padding:3px 6px;text-align:right;font-family:monospace;color:${r.p_value<0.05?'var(--green)':r.p_value<0.10?SQ.amber:'var(--muted)'}">${r.p_value.toFixed(3)}</td><td style="padding:3px 6px;text-align:right;color:var(--muted)">${r.n_windows}/${r.n_total}</td><td style="padding:3px 6px;text-align:right;color:${r.sign_pct>=70?'var(--green)':'var(--muted)'}">${r.sign_pct}%</td><td style="padding:3px 6px;text-align:right;font-family:monospace;color:var(--muted)">${r.std_err.toFixed(3)}</td><td style="padding:3px 6px;text-align:right;font-family:monospace;color:${r.instability<1?'var(--green)':r.instability<2?SQ.amber:'var(--red)'}">${r.instability.toFixed(2)}</td></tr>`).join('')}</tbody></table>`
    : `<table style="width:100%;border-collapse:collapse;margin-top:6px;font-size:11px"><tbody>${(fs.features||[]).map(f=>`<tr style="border-bottom:1px solid #161616"><td style="padding:3px 8px;font-family:monospace;color:#cbd5e1">${f.name}</td><td style="padding:3px 8px;color:var(--muted)">${f.category}</td><td style="padding:3px 8px;text-align:right;font-family:monospace;color:${f.coef>=0?'var(--green)':'var(--red)'}">${f.coef>=0?'+':''}${f.coef.toFixed(3)}</td></tr>`).join('')}</tbody></table>`;
  const fsHtml=(fs.n_total||fst.length)?`<div style="margin-top:18px;border-top:1px solid #222;padding-top:12px">
  ...
  const piHtml=pind.length?`
    <div style="margin:4px 0 16px;border:1px solid #2a3a5a;border-radius:6px;padding:12px;background:rgba(59,130,246,0.05)">
      ...
      <table style="width:100%;border-collapse:collapse;font-size:11.5px"><thead><tr style="color:var(--muted);text-align:left;border-bottom:1px solid #333"><th style="padding:4px 8px">Industry</th><th style="padding:4px 8px;text-align:right">Names</th><th style="padding:4px 8px;text-align:right" title="shrinkage weight (vc1.com): λ=own signal, 1−λ=global">λ</th><th style="padding:4px 8px;text-align:right">IC</th><th style="padding:4px 8px;text-align:right">Sharpe</th><th style="padding:4px 8px;text-align:right">Holdout</th><th style="padding:4px 8px">Current top picks</th></tr></thead><tbody>${pind.map((e,i)=>{const b=e.backtest||{},icv=e.ic.mean,sh=b.sharpe,ho=b.holdout_sharpe;const col=v=>v==null?'var(--muted)':(v>=0.5?'var(--green)':v>=0?SQ.amber:'var(--red)');return `<tr style="border-bottom:1px solid #1a1a1a;cursor:pointer" onclick="var x=document.getElementById('pi-${i}');x.style.display=x.style.display==='none'?'':'none'"><td style="padding:4px 8px;color:#e5e7eb;font-weight:600">${e.sector}</td><td style="padding:4px 8px;text-align:right;color:var(--muted)">${e.n_names}</td><td style="padding:4px 8px;text-align:right;font-family:monospace;color:#a78bfa">${e.lambda!=null?e.lambda.toFixed(2):'—'}</td><td style="padding:4px 8px;text-align:right;font-family:monospace;color:${icv>=0?'var(--green)':'var(--red)'}">${icv==null?'—':(icv>=0?'+':'')+icv.toFixed(3)}</td><td style="padding:4px 8px;text-align:right;font-family:monospace;color:${col(sh)}">${sh==null?'—':sh.toFixed(2)}</td><td style="padding:4px 8px;text-align:right;font-family:monospace;color:${col(ho)}">${ho==null?'—':ho.toFixed(2)}</td><td style="padding:4px 8px;color:#9ca3af;font-size:10.5px">${(e.top_picks||[]).slice(0,5).map(p=>p.ticker).join(', ')}</td></tr><tr id="pi-${i}" style="display:none;background:#0a0a0a"><td colspan="7" style="padding:6px 12px"><div style="display:flex;gap:24px;flex-wrap:wrap;font-size:10.5px"><div><span style="color:var(--green)">▲ ${e.sector} drivers:</span> ${(e.drivers.pos||[]).map(x=>x[0]+' '+x[1].toFixed(2)).join(' · ')}</div><div><span style="color:var(--red)">▼ drivers:</span> ${(e.drivers.neg||[]).map(x=>x[0]+' '+x[1].toFixed(2)).join(' · ')}</div></div><div style="font-size:10.5px;color:#9ca3af;margin-top:4px">all picks: ${(e.top_picks||[]).map(p=>p.ticker+' ('+(p.score>=0?'+':'')+p.score.toFixed(2)+')').join(' · ')}</div></td></tr>`;}).join('')}</tbody></table>
    </div>`:'';
